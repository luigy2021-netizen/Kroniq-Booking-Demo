...
st.write(
    "Demo de agenda digital para negocios que trabajan por cita."
)
...
if plan_corto in ("Business", "Premium"):
    st.markdown(
        ...
        ">
            ...
            <p style="margin:0">
                Promoción de ejemplo visible en el encabezado
                de tu agenda.
            </p>
        </div>
        """,
...
if plan_corto == "Premium":
    st.markdown(
        ...
        ">
            ...
            Reserva dos servicios este mes y recibe
    ...
    if accion_demo == "Reprogramar una cita":
        st.success(
            "La cita puede moverse a otro horario disponible "
            "sin crear empalmes."
        )

    elif accion_demo == "Cancelar una cita":
        st.warning(
            "La cita se cancela y el horario vuelve "
            "a quedar disponible."
        )
        ...
        st.info(
            "Durante una implementación real, esa fecha "
            "dejaría de aceptar reservaciones."
        )
...
if enviar:
    ...
    if not nombre:
    ...
    elif hora not in horarios_disponibles(
        fecha,
        duracion,
        giro,
    ):
        ...
        try:
            ...
            st.success(
                "✅ Cita demo guardada correctamente."
            )

            st.info(
                f"Confirmación automática de ejemplo: "
                f"Hola, {nombre}. Tu cita de {servicio} "
                f"quedó agendada para el "
            ...
            if plan_corto in ("Business", "Premium"):
                st.success(
                    "También se generó una recomendación "
                    "para compartir la agenda."
                )
...
if plan_corto in ("Business", "Premium"):
    ...
    with izquierda:
        st.image(
            crear_qr(
                "https://kroniq-booking-demo.streamlit.app"
            ),
    ...
    with derecha:
        ...
        st.write(
            "El negocio puede imprimirlo, publicarlo "
            "o enviarlo para que sus clientes abran "
            "la agenda."
        )

        st.code(
            "Recomienda esta agenda y comparte el QR",
            language=None,
        )

else:
    st.write(
        "El código QR para compartir la agenda "
        "está disponible a partir del plan Business."
    )
...
st.caption(
    "KroniQ Booking — agenda digital para negocios modernos."
)
